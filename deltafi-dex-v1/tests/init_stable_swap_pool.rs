...
use deltafi_swap::{
    error::SwapError,
    instruction::{stable_initialize, StableInitializeData},
    math::{Decimal, TryDiv},
    processor::process,
    state::SwapType,
};
...
async fn test_success() {
    ...
    let usdc_admin_account = create_and_mint_to_token_account(
        &mut banks_client,
        usdc_mint.pubkey,
        Some(&usdc_mint.authority),
        &payer,
    ...
    .await;
    let usdt_admin_account = create_and_mint_to_token_account(
        &mut banks_client,
        usdt_mint.pubkey,
        Some(&usdt_mint.authority),
        &payer,
